...
Main Table:
 ...
 CREATE TABLE cachedmodule (id integer primary key default nextval('cachedmodule_seq'),path text not null  ,version text not null  ,suffix text not null  ,key text not null  ,created integer not null  ,lastused integer not null  );
...
Archive Table: (structs can be moved from main to archive using Archive() function)
 ...
 CREATE TABLE cachedmodule_archive (id integer unique not null,path text not null,version text not null,suffix text not null,key text not null,created integer not null,lastused integer not null);
*/
...
func (a *DBCachedModule) Archive(ctx context.Context, id uint64) error {
	...
	_, e := a.DB.ExecContext(ctx, "archive_DBCachedModule", "insert into "+a.SQLArchivetablename+" (id,path, version, suffix, key, created, lastused) values ($1,$2, $3, $4, $5, $6, $7) ", p.ID, p.Path, p.Version, p.Suffix, p.Key, p.Created, p.LastUsed)
	if e != nil {
		return e
	}
...
func (a *DBCachedModule) Save(ctx context.Context, p *savepb.CachedModule) (uint64, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "insert into "+a.SQLTablename+" (path, version, suffix, key, created, lastused) values ($1, $2, $3, $4, $5, $6) returning id", p.Path, p.Version, p.Suffix, p.Key, p.Created, p.LastUsed)
	if e != nil {
...
func (a *DBCachedModule) SaveWithID(ctx context.Context, p *savepb.CachedModule) error {
	...
	_, e := a.DB.ExecContext(ctx, qn, "insert into "+a.SQLTablename+" (id,path, version, suffix, key, created, lastused) values ($1,$2, $3, $4, $5, $6, $7) ", p.ID, p.Path, p.Version, p.Suffix, p.Key, p.Created, p.LastUsed)
	return a.Error(ctx, qn, e)
}

func (a *DBCachedModule) Update(ctx context.Context, p *savepb.CachedModule) error {
	qn := "DBCachedModule_Update"
	_, e := a.DB.ExecContext(ctx, qn, "update "+a.SQLTablename+" set path=$1, version=$2, suffix=$3, key=$4, created=$5, lastused=$6 where id = $7", p.Path, p.Version, p.Suffix, p.Key, p.Created, p.LastUsed, p.ID)
...
func (a *DBCachedModule) ByID(ctx context.Context, p uint64) (*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where id = $1", p)
	if e != nil {
...
func (a *DBCachedModule) All(ctx context.Context) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" order by id")
	if e != nil {
...
func (a *DBCachedModule) ByPath(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where path = $1", p)
	if e != nil {
...
func (a *DBCachedModule) ByLikePath(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where path ilike $1", p)
	if e != nil {
		return nil, a.Error(ctx, qn, fmt.Errorf("ByPath: error querying (%s)", e))
	}
	defer rows.Close()
...
func (a *DBCachedModule) ByVersion(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where version = $1", p)
	if e != nil {
...
func (a *DBCachedModule) ByLikeVersion(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where version ilike $1", p)
	if e != nil {
		return nil, a.Error(ctx, qn, fmt.Errorf("ByVersion: error querying (%s)", e))
	}
	defer rows.Close()
...
func (a *DBCachedModule) BySuffix(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where suffix = $1", p)
	if e != nil {
...
func (a *DBCachedModule) ByLikeSuffix(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where suffix ilike $1", p)
	if e != nil {
		return nil, a.Error(ctx, qn, fmt.Errorf("BySuffix: error querying (%s)", e))
	}
	defer rows.Close()
...
func (a *DBCachedModule) ByKey(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where key = $1", p)
	if e != nil {
...
func (a *DBCachedModule) ByLikeKey(ctx context.Context, p string) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where key ilike $1", p)
	if e != nil {
		return nil, a.Error(ctx, qn, fmt.Errorf("ByKey: error querying (%s)", e))
	}
	defer rows.Close()
...
func (a *DBCachedModule) ByCreated(ctx context.Context, p uint32) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where created = $1", p)
	if e != nil {
...
func (a *DBCachedModule) ByLikeCreated(ctx context.Context, p uint32) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where created ilike $1", p)
	if e != nil {
		return nil, a.Error(ctx, qn, fmt.Errorf("ByCreated: error querying (%s)", e))
	}
	defer rows.Close()
...
func (a *DBCachedModule) ByLastUsed(ctx context.Context, p uint32) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where lastused = $1", p)
	if e != nil {
...
func (a *DBCachedModule) ByLikeLastUsed(ctx context.Context, p uint32) ([]*savepb.CachedModule, error) {
	...
	rows, e := a.DB.QueryContext(ctx, qn, "select id,path, version, suffix, key, created, lastused from "+a.SQLTablename+" where lastused ilike $1", p)
	if e != nil {
		return nil, a.Error(ctx, qn, fmt.Errorf("ByLastUsed: error querying (%s)", e))
	}
	defer rows.Close()
...
func (a *DBCachedModule) SelectCols() string {
	return "id,path, version, suffix, key, created, lastused"
}
func (a *DBCachedModule) SelectColsQualified() string {
	return "" + a.SQLTablename + ".id," + a.SQLTablename + ".path, " + a.SQLTablename + ".version, " + a.SQLTablename + ".suffix, " + a.SQLTablename + ".key, " + a.SQLTablename + ".created, " + a.SQLTablename + ".lastused"
}
...
func (a *DBCachedModule) FromRows(ctx context.Context, rows *gosql.Rows) ([]*savepb.CachedModule, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&foo.ID, &foo.Path, &foo.Version, &foo.Suffix, &foo.Key, &foo.Created, &foo.LastUsed)
		if err != nil {
...
func (a *DBCachedModule) CreateTable(ctx context.Context) error {
	csql := []string{
		...
		`CREATE TABLE if not exists ` + a.SQLTablename + ` (id integer primary key default nextval('` + a.SQLTablename + `_seq'),path text not null  ,version text not null  ,suffix text not null  ,key text not null  ,created integer not null  ,lastused integer not null  );`,
		`CREATE TABLE if not exists ` + a.SQLTablename + `_archive (id integer primary key default nextval('` + a.SQLTablename + `_seq'),path text not null  ,version text not null  ,suffix text not null  ,key text not null  ,created integer not null  ,lastused integer not null  );`,
	}
	for i, c := range csql {
